...
import (
	...
	dexUtils "github.com/fastock/fastock-chain/x/dex/client/utils"
...
$ fastock-chain-cli tx dex list --base-asset mytoken --quote-asset okt --from mykey
`),
...
$ fastock-chain-cli tx dex deposit mytoken_okt 1000okt --from mykey
...
$ fastock-chain-cli tx dex withdraw mytoken_okt 1000okt --from mykey
...
$ fastock-chain-cli tx dex register-operator --website http://xxx/operator.json --handling-fee-address addr --from mykey
`),
		RunE: func(cmd *cobra.Command, _ []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
...
$ fastock-chain-cli tx dex edit-operator --website http://xxx/operator.json --handling-fee-address addr --from mykey
`),
		RunE: func(cmd *cobra.Command, _ []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
