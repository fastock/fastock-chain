...
import (
	...
	"github.com/fastock/fastock-chain/x/debug/types"
...
func CmdSetLogLevel(queryRoute string, cdc *codec.Codec) *cobra.Command {
	return &cobra.Command{
		Use:   "set-loglevel",
		Args:  cobra.ExactArgs(1),
		Short: "Set the fastock-chain-daemon log level",
		Long: strings.TrimSpace(`
$ fastock-chain-cli debug set-loglevel "main:info,state:info"

$ fastock-chain-cli debug set-loglevel "upgrade:error"
`),
		RunE: func(cmd *cobra.Command, args []string) error {
			cliCtx := context.NewCLIContext().WithCodec(cdc)
			if len(args) > 1 {
			...
			fmt.Println("Succeed to set the fastock-chain-daemon log level.")
			return nil
		},
	}
}
